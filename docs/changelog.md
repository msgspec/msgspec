...
- Fix a reference leak when defining a `Struct` with a non-`Struct` base on
  Python 3.12+: the base's type dict was never released, keeping the base
  class alive on 3.12 through 3.14 ({pr}`1199`).
